...
let vkOverallChartInstance = null;
const createEChart = (element, currentInstance) => {
  if (!element || !window.echarts) return null;
  if (currentInstance && !currentInstance.isDisposed()) currentInstance.dispose();
  const attachedInstance = window.echarts.getInstanceByDom(element);
  if (attachedInstance && !attachedInstance.isDisposed()) attachedInstance.dispose();
  return window.echarts.init(element, null, { renderer: "canvas" });
};
const resizeOverallCharts = () => {
  [
    bookrollOverallChartInstance,
    videoOverallChartInstance,
    chatbotQuizOverallChartInstance,
    vkOverallChartInstance
  ].forEach((chart) => {
    if (chart && !chart.isDisposed()) chart.resize();
  });
};
let overallChartResizeFrame = null;
window.addEventListener("resize", () => {
  if (overallChartResizeFrame) cancelAnimationFrame(overallChartResizeFrame);
  overallChartResizeFrame = requestAnimationFrame(resizeOverallCharts);
});
let userId = null;
...
const chatbotProfileUrl = (email, cid) =>
  `${STUDENT_CONFIG.baseUrl || "https://adaptive-profile-bn.ae.app.meca.in.th"}/api/kidbright/course/${encodeURIComponent(cid)}/data/chatbot?email=${encodeURIComponent(email)}`;

const chatbotScoreV2Url = (cid) =>
  `https://sbs-backend.mooc.meca.in.th/me/data/chatbot/${encodeURIComponent(cid)}`;

const vkAnalysisBaseUrl = () =>
  String(STUDENT_CONFIG.vk?.baseUrl || "https://vk-analysis.learning.app.meca.in.th").replace(/\/+$/, "");

const vkAnalysisOverviewUrl = (uid, cid, chapterNo = null) => {
  const base = `${vkAnalysisBaseUrl()}/analysis/overview/${encodeURIComponent(uid)}/course/${encodeURIComponent(cid)}`;
  return chapterNo == null ? base : `${base}/chapter/${encodeURIComponent(chapterNo)}`;
};
...
const getVkSeriesMap = (payload) => {
  const map = new Map();
  const series = Array.isArray(payload?.Params?.series) ? payload.Params.series : [];
  series.forEach((entry) => {
    const name = String(entry?.name || "").trim();
    if (!name) return;
    map.set(name, Array.isArray(entry?.data) ? entry.data : []);
  });
  return map;
};

const getVkSkillResult = (payload, skillName = "VK skill") => {
  const series = Array.isArray(payload?.[skillName]?.series) ? payload[skillName].series : [];
  const result = series.find((entry) => String(entry?.name || "").trim().toLowerCase() === "result");
  const value = Array.isArray(result?.data) ? result.data[0] : null;
  return value == null ? "" : String(value).trim();
};

const normalizeVkChapterItems = (payload, chapterNo) => {
  const itemCodes = Array.isArray(payload?.Params?.x_data) ? payload.Params.x_data : [];
  const locations = Array.isArray(payload?.x_locate) ? payload.x_locate : [];
  const seriesMap = getVkSeriesMap(payload);
  const chapterTitle = Array.isArray(payload?.x_title)
    ? String(payload.x_title[0] || "")
    : String(payload?.x_title || "");
  const result = getVkSkillResult(payload);

  return itemCodes.map((rawCode, index) => {
    const metrics = {};
    seriesMap.forEach((values, name) => {
      const value = values[index];
      const numeric = value == null || value === "" ? null : Number(value);
      metrics[name] = Number.isFinite(numeric) ? numeric : value;
    });
    const activityMetricNames = [
      "Time used", "Coding mark", "Coding time", "Solution mark", "Scene used",
      "Object used", "Image used", "Snap count", "Label used", "AIBlock time",
      "AIBlock used", "Capture time", "Annotate time", "VK time", "Switch count", "Train time"
    ];
    const hasActivity = activityMetricNames.some((name) => Number(metrics[name]) > 0);
    return {
      itemCode: String(rawCode || "").trim().toLowerCase(),
      chapterNo: Number(chapterNo),
      title: decodeIfMojibake(chapterTitle),
      locate: Array.isArray(locations[index]) ? locations[index] : [],
      metrics,
      result,
      hasActivity,
      raw: payload
...
const getVkItemData = (itemCode, candidateTitles = []) => {
  const entries = Array.isArray(window.vkItemData) ? window.vkItemData : [];
  const code = String(itemCode || "").trim().toLowerCase();
  if (code) {
    const exact = entries.find((entry) => entry?.itemCode === code);
    if (exact) return exact;
  }
  const keys = (Array.isArray(candidateTitles) ? candidateTitles : [candidateTitles])
    .map((title) => normalizeTopicKey(title))
    .filter(Boolean);
  if (!keys.length) return null;
  return entries.find((entry) => {
    const titleKey = normalizeTopicKey(entry?.title);
    return titleKey && keys.some((key) => titleKey === key || titleKey.includes(key) || key.includes(titleKey));
  }) || null;
};

const formatVkMinutes = (value) => {
  const minutes = Number(value);
  if (!Number.isFinite(minutes) || minutes < 0) return "";
  if (minutes === 0) return "0 นาที";
  if (minutes < 1) return `${Math.round(minutes * 60)} วินาที`;
  return `${minutes.toLocaleString("th-TH", { maximumFractionDigits: 1 })} นาที`;
};

const formatVkMark = (value) => {
...
const updateLoginDebugPanel = () => {
  ...
  if (loginDebugTabDerivedEl) {
    const rows = [
      ...
      { name: "VK Analysis", param: userId && courseId ? vkAnalysisOverviewUrl(userId, courseId) : "-" }
    ];
...
const chatbotRoleLabel = (role) => {
  const value = String(role || "").toLowerCase();
  if (value === "pretest") return "ก่อนเรียน";
  if (value === "posttest") return "หลังเรียน";
  return "แบบทดสอบ";
};

const normalizeChatbotProfileData = (payload) => {
  const collections = Array.isArray(payload?.collections) ? payload.collections : [];
  const rows = [];
  collections.forEach((collection) => {
    const quizzes = Array.isArray(collection?.quizzes) ? collection.quizzes : [];
    quizzes.forEach((quiz) => {
      const attemptHistory = Array.isArray(quiz?.attempt_history) ? quiz.attempt_history : [];
      const attemptCount = pickNumber(quiz, ["total_attempts"]);
      const attempted =
        (Number.isFinite(attemptCount) && attemptCount > 0) ||
        attemptHistory.length > 0 ||
        Boolean(quiz?.first_attempt_at || quiz?.last_attempt_at);
      if (!attempted) return;

      const bestAttempt = attemptHistory.reduce((best, attempt) => {
        const score = pickNumber(attempt, ["correct_count"]);
        const bestScore = pickNumber(best, ["correct_count"]);
        if (!best || (Number.isFinite(score) && (!Number.isFinite(bestScore) || score > bestScore))) return attempt;
        return best;
      }, null);
      const latestAttempt = attemptHistory[attemptHistory.length - 1] || null;
      const total = pickNumber(quiz, ["best_total_count", "max_questions"])
        ?? pickNumber(bestAttempt, ["total_count"])
        ?? pickNumber(latestAttempt, ["total_count"]);
      const bestScorePct = pickNumber(quiz, ["best_score_pct", "latest_score_pct"]);
      const scoreYou = pickNumber(quiz, ["best_correct_count"])
        ?? pickNumber(bestAttempt, ["correct_count"])
        ?? (Number.isFinite(total) && Number.isFinite(bestScorePct) ? total * bestScorePct / 100 : null);
      const peerPct = pickNumber(quiz?.quiz_peer_comparison, ["cohort_avg_score_pct"])
        ?? pickNumber(quiz, ["avg_score_pct"]);
      const scoreAvg = Number.isFinite(total) && Number.isFinite(peerPct)
        ? total * peerPct / 100
        : null;
      const timeYou = pickNumber(quiz, ["avg_time_seconds"])
        ?? pickNumber(latestAttempt, ["total_time_seconds"]);
      const role = String(quiz?.quiz_role || "").toLowerCase();
      const percentile = pickNumber(quiz?.quiz_peer_comparison, ["learner_percentile"]);
      const cohortLearners = pickNumber(quiz?.quiz_peer_comparison, ["total_learners_attempted"]);

      rows.push({
        title: decodeIfMojibake(String(quiz?.title || collection?.title || `Quiz ${rows.length + 1}`)),
        chartTitle: chatbotRoleLabel(role),
        role,
        total: Number.isFinite(total) && total > 0 ? total : null,
        scoreYou: Number.isFinite(scoreYou) ? scoreYou : null,
        scoreAvg: Number.isFinite(scoreAvg) ? scoreAvg : null,
        bestScorePct: Number.isFinite(bestScorePct) ? bestScorePct : null,
        latestScorePct: pickNumber(quiz, ["latest_score_pct"]),
        learnerAveragePct: pickNumber(quiz, ["avg_score_pct"]),
        cohortAveragePct: Number.isFinite(peerPct) ? peerPct : null,
        timeYou: Number.isFinite(timeYou) ? Math.max(0, timeYou) : null,
        attempts: Number.isFinite(attemptCount) ? attemptCount : attemptHistory.length,
        percentile: Number.isFinite(percentile) ? percentile : null,
        cohortLearners: Number.isFinite(cohortLearners) ? cohortLearners : null,
        collectionTitle: decodeIfMojibake(String(collection?.title || "")),
        completionPct: pickNumber(collection, ["completion_pct"]),
        lastAttemptAt: quiz?.last_attempt_at || latestAttempt?.completed_at || null,
        usageId: String(quiz?.quiz_id || quiz?.share_code || collection?.collection_id || "")
      });
    });
  });

  if (!rows.length) return { rows, performance: null, speed: null };

  const categoryMeta = rows.map((row) => ({
    raw: row.chartTitle,
    key: row.chartTitle,
    title: row.chartTitle,
    fullTitle: row.title,
    total: row.total,
    usageId: row.usageId,
    source: "profile",
    ...
    latestScorePct: row.latestScorePct,
    learnerAveragePct: row.learnerAveragePct,
    cohortAveragePct: row.cohortAveragePct,
    percentile: row.percentile,
    cohortLearners: row.cohortLearners,
  ...
  const axis = {
    type: "category",
    data: rows.map((row) => row.chartTitle)
  };
  return {
    rows,
    performance: {
      xAxis: axis,
      series: [
        { name: "Your Score", data: rows.map((row) => row.scoreYou) },
        { name: "Average Score", data: rows.map((row) => row.scoreAvg) }
      ],
      __categoryMeta: categoryMeta
    },
    speed: {
      xAxis: axis,
      series: [
        { name: "Your Time", data: rows.map((row) => row.timeYou) }
      ],
      __categoryMeta: categoryMeta
    }
  };
...
const formatChatbotNumber = (value, maximumFractionDigits = 1) => {
  const number = Number(value);
  if (!Number.isFinite(number)) return "-";
  return new Intl.NumberFormat("th-TH", { maximumFractionDigits }).format(number);
};

const formatChatbotScore = (info, maximumFractionDigits = 0) => {
  if (!info || !Number.isFinite(info.score)) return "-";
  const scoreText = formatChatbotNumber(info.score, maximumFractionDigits);
  if (!Number.isFinite(info.total) || info.total <= 0) return scoreText;
  return `${scoreText}/${formatChatbotNumber(info.total, 0)}`;
};

const hasChatbotAttemptStarted = (item) => {
  const attempts = Number(item?.chatbotAttempts ?? item?.attempts);
...
const buildChatbotOrderMap = (course, perfCfg, speedCfg) => {
  ...
  const isProfileData = perfMeta.some((meta) => Number.isFinite(Number(meta?.attempts)));
  ...
  let apiCursor = 0;
  const mappedChapters = chapters.map((chapterNode, chapterIdx) => {
      ...
      const items = slots.map((slot, slotIdx) => {
        ...
        const apiMeta = apiIdx != null ? getCategoryMetaAt(perfCfg, apiIdx) : null;
        ...
        return {
          ...
          apiIdx,
          id: slot.id,
          title: isProfileData && apiMeta?.title ? apiMeta.title : slot.title,
          ...
          attempts: apiIdx != null ? Number(apiMeta?.attempts) || null : null,
          profile: apiMeta?.source === "profile" ? apiMeta : null
        };
      });
      return { chapterIdx, items };
    });

  if (isProfileData && apiCursor < maxLen) {
    let targetChapter = [...mappedChapters].reverse().find((chapter) => chapter.items.length);
    if (!targetChapter && mappedChapters.length) targetChapter = mappedChapters[0];
    while (targetChapter && apiCursor < maxLen) {
      const apiIdx = apiCursor;
      const apiMeta = getCategoryMetaAt(perfCfg, apiIdx) || {};
      targetChapter.items.push({
        chapterIdx: targetChapter.chapterIdx,
        slotIdx: targetChapter.items.length,
        apiIdx,
        id: apiMeta.usageId || "",
        title: apiMeta.title || apiMeta.key || apiMeta.raw || `Quiz ${apiIdx + 1}`,
        verticalTitle: "",
        totalHint: Number(apiMeta.total) || null,
        scoreYou: getChatbotScoreAt(perfCfg, apiIdx, "your"),
        scoreAvg: getChatbotScoreAt(perfCfg, apiIdx, "average"),
        timeYou: getChatbotTimeAt(speedCfg, apiIdx, "your"),
        timeAvg: getChatbotTimeAt(speedCfg, apiIdx, "average"),
        attempts: Number(apiMeta.attempts) || null,
        profile: apiMeta.source === "profile" ? apiMeta : null
      });
      apiCursor += 1;
    }
...
const extractVkItemCode = (raw) => {
  const text = decodeIfMojibake(String(raw || ""));
  if (!text) return "";
  const queryMatch = text.match(/[?&]item=([^&#\s"']+)/i);
  if (queryMatch?.[1]) {
    try { return decodeURIComponent(queryMatch[1]).trim().toLowerCase(); } catch {}
    return String(queryMatch[1]).trim().toLowerCase();
  }
  const codeMatch = text.match(/\bvk\d[\w-]*\b/i);
  return codeMatch?.[0] ? codeMatch[0].toLowerCase() : "";
};

const isVkToolText = (raw) => {
  const text = decodeIfMojibake(String(raw || "")).toLowerCase();
  if (!text) return false;
  return (
    text.includes("vk-kbai.learning.app.meca.in.th") ||
    text.includes("vk-analysis.learning.app.meca.in.th") ||
    text.includes("/ae/exam?item=vk") ||
    /\bvk\d[\w-]*\b/i.test(text)
  );
};
...
const collectVerticalAeSubtools = (vertical) => {
  ...
  while (stack.length) {
    ...
    if (isAe) {
      ...
      if (subtype === "BookRoll" || subtype === "Video" || subtype === "Chatbot" || subtype === "VK") {
        ...
        if (!seen.has(key)) {
          ...
          const rawMeta = [
            ...
            n?.fields?.href,
            n?.fields?.src,
            n?.fields?.data?.iframe_url,
            JSON.stringify(n?.fields?.data || {})
          ].filter(Boolean).join("\n");
          out.push({
            id: id || "",
            subtype,
            title: decodeIfMojibake(String(n?.title || n?.fields?.display_name || "")),
            itemCode: subtype === "VK" ? extractVkItemCode(rawMeta) : ""
          });
        }
      }
    }
...
const normalizeTrackedToolKey = (rawSubtype) => {
  ...
  if (t === "vk" || t.includes("vk tool")) return "vk";
  return "";
};

const collectCourseToolInventory = (course) => {
  const counts = { bookroll: 0, video: 0, chatbot: 0, vk: 0 };
...
const syncOverallToolCards = () => {
  ...
  const vkEl = document.getElementById("overall-card-vk");
  ...
  if (vkEl) vkEl.classList.toggle("hidden", !visibility.vk);

  const visibleCount = Object.values(visibility).filter(Boolean).length;
  containerEl.classList.toggle("hidden", visibleCount === 0);
  containerEl.classList.remove("lg:grid-cols-1", "lg:grid-cols-2", "lg:grid-cols-3", "xl:grid-cols-4");
  if (visibleCount >= 4) containerEl.classList.add("lg:grid-cols-2", "xl:grid-cols-4");
  else if (visibleCount === 3) containerEl.classList.add("lg:grid-cols-3");
  ...
  const out = chapters.map((chapterNode, chapterIdx) => {
    ...
    const seqItems = sortSequentialItemsHeuristic(sequentials.map((seq, sIdx) => {
      ...
      const verts = sortVerticalItemsHeuristic(verticals.map((v, vIdx) => {
        ...
        const subtools = collectVerticalAeSubtools(v)
          .map((entry) => {
            ...
            const chatbotItem = entry.subtype === "Chatbot"
              ...
              : null;
            const vk = entry.subtype === "VK"
              ? getVkItemData(entry.itemCode, [entryTitle, rawEntryTitle, verticalTitle])
              : null;
            ...
            return {
              ...
              chatbotAttempts: chatbotItem?.attempts ?? null,
              chatbotProfile: chatbotItem?.profile || null
            };
          });
        ...
        const baseToolSubtype = String(baseTool?.sublabel || "").toLowerCase();
        const verticalVk = baseToolSubtype === "vk"
          ? getVkItemData(extractVkItemCode(findAeToolMeta(v)?.iframeUrl), [pickTitle(v, `Vertical ${vIdx + 1}`)])
          : null;
...
const updateStatusFromModel = (model) => {
  ...
  renderVkOverview(window.vkOverviewRawData);
};

const renderStatusDonut = (elementId, currentInstance, stats, labels) => {
  const element = document.getElementById(elementId);
  const chart = createEChart(element, currentInstance);
  if (!chart) return null;

  const values = [
    Math.max(0, Number(stats?.done) || 0),
    Math.max(0, Number(stats?.doing) || 0),
    Math.max(0, Number(stats?.todo) || 0)
  ];
  const total = values.reduce((sum, value) => sum + value, 0);
  const data = total > 0
    ? [
        { name: labels[0], value: values[0], itemStyle: { color: "#18b879" } },
        { name: labels[1], value: values[1], itemStyle: { color: "#4f8cff" } },
        { name: labels[2], value: values[2], itemStyle: { color: "#94a3b8" } }
      ]
    : [{ name: "ยังไม่มีข้อมูล", value: 1, itemStyle: { color: "#e2e8f0" } }];
  ...
  chart.setOption({
    ...
    legend: {
      ...
      itemWidth: 10,
      itemHeight: 10,
      textStyle: { color: "#5b6b6f", fontSize: 11 }
    },
    series: [{
      type: "pie",
      radius: ["48%", "69%"],
...
const renderBookrollStatusDonut = (book) => {
  bookrollOverallChartInstance = renderStatusDonut(
    "bookrollOverallChart",
    bookrollOverallChartInstance,
    book,
    ["อ่านจบ", "กำลังอ่าน", "ยังไม่ได้อ่าน"]
  );
};

const renderVideoStatusDonut = (video) => {
  videoOverallChartInstance = renderStatusDonut(
    "videoOverallChart",
    videoOverallChartInstance,
    video,
    ["ดูจบ", "กำลังดู", "เหลือ"]
...
const renderChatbotQuizOverview = (chatbotMap) => {
  const element = document.getElementById("chatbotQuizOverallChart");
  const summaryEl = document.getElementById("chatbot-quiz-summary");
  if (!element) return;
  chatbotQuizOverallChartInstance = createEChart(element, chatbotQuizOverallChartInstance);
  if (!chatbotQuizOverallChartInstance) return;
  ...
  const rows = items
    .map((item, idx) => {
      ...
      const rawScoreYou = item?.scoreYou?.score;
      const rawScoreAvg = item?.scoreAvg?.score;
      const scoreYou = rawScoreYou == null || rawScoreYou === "" ? null : Number(rawScoreYou);
      const scoreAvg = rawScoreAvg == null || rawScoreAvg === "" ? null : Number(rawScoreAvg);
      ...
      return {
        ...
        hasData:
          Number.isFinite(scoreYou) ||
          Number.isFinite(scoreAvg) ||
          Number.isFinite(Number(item?.timeYou)) ||
          Number(item?.attempts) > 0
      };
    })
    .filter((row) => row.label && row.hasData);

  if (!rows.length) {
    chatbotQuizOverallChartInstance.setOption({
  ...
  const profileRows = rows.filter((row) => row.profile?.source === "profile");
  ...
  const series = [
    {
      ...
      barMaxWidth: 28,
      itemStyle: { color: "#0f766e", borderRadius: [7, 7, 0, 0] }
    },
    {
      name: "ค่าเฉลี่ย",
      type: "bar",
      ...
      barMaxWidth: 28,
      itemStyle: { color: "#38bdf8", borderRadius: [7, 7, 0, 0] }
    }
  ];
  if (hasAnyTotals) {
    series.push({
      ...
      barMaxWidth: 28,
      itemStyle: { color: "#cbd5e1", borderRadius: [7, 7, 0, 0] }
    });
  }
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        const details = (Array.isArray(params) ? params : [])
          .map((item) => `${item.seriesName}: ${item.value == null ? "-" : formatChatbotNumber(item.value)}`)
          .join("\n");
  ...
  if (summaryEl) {
    if (profileRows.length === rows.length) {
      const totalAttempts = profileRows.reduce((sum, row) => sum + (Number.isFinite(row.attempts) ? row.attempts : 0), 0);
      const cohortAverage = profileRows.length === 1
        ? Number(profileRows[0].profile?.cohortAveragePct)
        : null;
      const percentile = profileRows.length === 1
        ? Number(profileRows[0].profile?.percentile)
        : null;
      const parts = [
        `มี ${rows.length} แบบทดสอบ`,
        totalAttempts > 0 ? `ทำ ${formatChatbotNumber(totalAttempts, 0)} ครั้ง` : "",
        hasAllTotals && totalScore > 0
          ? `คะแนนดีที่สุด ${formatChatbotNumber(totalYou, 0)}/${formatChatbotNumber(totalScore, 0)}`
          : "",
        Number.isFinite(cohortAverage) ? `ค่าเฉลี่ยกลุ่ม ${formatChatbotNumber(cohortAverage)}%` : "",
        Number.isFinite(percentile) ? `เปอร์เซ็นไทล์ ${formatChatbotNumber(percentile)}` : ""
      ].filter(Boolean);
...
const renderVkOverview = (payload) => {
  const element = document.getElementById("vkOverallChart");
  const summaryEl = document.getElementById("vk-overall-summary");
  if (!element) return;
  vkOverallChartInstance = createEChart(element, vkOverallChartInstance);
  if (!vkOverallChartInstance) return;

  const xData = Array.isArray(payload?.Params?.x_data) ? payload.Params.x_data : [];
  const rawTitles = Array.isArray(payload?.x_title) ? payload.x_title : [];
  const labels = xData.map((value, index) =>
    String(rawTitles[index] || `บทวิเคราะห์ ${value || index + 1}`)
  );
  const seriesMap = getVkSeriesMap(payload);
  const normalizeScores = (name) => {
  ...
  const coding = normalizeScores("Coding mark");
  const solution = normalizeScores("Solution mark");
  const activityNames = ["Time used", "Coding time", "VK time", "Switch count", "Coding mark", "Solution mark"];
  const hasActivity = activityNames.some((name) =>
    (seriesMap.get(name) || []).some((value) => Number(value) > 0)
  );

  if (!payload || !labels.length || !hasActivity) {
    vkOverallChartInstance.setOption({
      animationDuration: 350,
      textStyle: { fontFamily: "\"Kanit\", \"Noto Sans Thai\", sans-serif" },
      graphic: [{
        type: "text",
        left: "center",
        top: "middle",
        silent: true,
        style: {
          text: "ยังไม่มีข้อมูลการใช้งาน VK",
          fill: "#94a3b8",
          font: "500 13px Kanit, sans-serif",
          textAlign: "center"
        }
    ...
    if (summaryEl) summaryEl.textContent = "ยังไม่มีข้อมูลการใช้งาน VK";
    return;
  }
  ...
  vkOverallChartInstance.setOption({
    animationDuration: 450,
    textStyle: { fontFamily: "\"Kanit\", \"Noto Sans Thai\", sans-serif" },
    grid: { top: 18, right: 12, bottom: 60, left: 38, containLabel: true },
    tooltip: {
      trigger: "axis",
      renderMode: "richText",
      axisPointer: { type: "shadow" },
      formatter: (params) => {
        ...
        const details = (Array.isArray(params) ? params : [])
          .map((item) => `${item.seriesName}: ${item.value == null ? "-" : `${item.value}%`}`)
          .join("\n");
        return [title, details].filter(Boolean).join("\n");
      }
    },
    ...
    xAxis: {
      type: "category",
      data: labels.map((label) => shortenChartLabel(label, 18)),
      axisTick: { show: false },
      axisLine: { lineStyle: { color: "#d7dee5" } },
    ...
    series: [
      {
        ...
        data: coding,
        barMaxWidth: 28,
        itemStyle: { color: "#8b5cf6", borderRadius: [7, 7, 0, 0] }
      },
      {
        name: "Solution",
        type: "bar",
        data: solution,
        barMaxWidth: 28,
        itemStyle: { color: "#22c55e", borderRadius: [7, 7, 0, 0] }
      }
    ]
  }, true);

  if (summaryEl) {
    const done = Number(payload?.done?.value);
    const max = Number(payload?.done?.max);
    const timeValues = seriesMap.get("Time used") || [];
    const totalMinutes = timeValues.reduce((sum, value) => sum + (Number(value) || 0), 0);
    const result = getVkSkillResult(payload);
    const parts = [
      Number.isFinite(done) && Number.isFinite(max) && max > 0 ? `สำเร็จ ${done}/${max} หัวข้อ` : "",
      totalMinutes > 0 ? `ใช้เวลา ${formatVkMinutes(totalMinutes)}` : "",
      result ? `ผลประเมิน ${result}` : ""
    ].filter(Boolean);
    summaryEl.textContent = parts.join(" • ") || "มีข้อมูลการใช้งาน VK";
  }
};
  ...
  const blocks = sequentials.map((seq) => {
    const vHtml = (seq.verticals || []).map((v) => {
      ...
      const hasChatbotMetrics = subtoolList.some((item) =>
        ...
        Number(item?.chatbotAttempts) > 0
      );
      const hasChatbotStarted = subtoolList.some((item) => hasChatbotAttemptStarted(item));
      const hasVkTool = subtoolList.some((item) => String(item?.tool?.sublabel || "").toLowerCase() === "vk");
      const hasVkActivity = subtoolList.some((item) => item?.vk?.hasActivity);
        const badge = !hasPct
        ? (
          hasChatbotTool
            ? (
            ...
            : hasVkTool
              ? (hasVkActivity
                ? `<span class="badge badge-doing">มีข้อมูลการใช้งาน</span>`
      ...
      const renderToolIcon = (tool) => {
        ...
        if (subLc === "vk" || subLc.includes("vk tool")) {
          return `<span class="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-violet-100 text-violet-700 border border-violet-200 font-extrabold text-[10px]" aria-label="VK Tool">VK</span>`;
        }
        return "";
      };
      const visibleToolItems = subtoolList.filter((item) => {
        ...
        const hasChatbotData =
          ...
          Number.isFinite(item?.chatbotTimeAvg) ||
          Number(item?.chatbotAttempts) > 0;
        const hasVkData = !!item?.vk;
        const hasApiData = itemHasPct || !!item?.reading || !!item?.video || hasChatbotData || hasVkData;
      ...
      const toolItems = visibleToolItems.map((item) => {
        ...
        const isChatbot = String(itemTool?.sublabel || "").toLowerCase() === "chatbot";
        const isVk = String(itemTool?.sublabel || "").toLowerCase() === "vk";
        ...
        const vk = item.vk || null;
        const chatbotProfile = item.chatbotProfile?.source === "profile" ? item.chatbotProfile : null;
        const chatbotScoreMain = chatbotStarted && item.chatbotScore ? formatChatbotScore(item.chatbotScore) : "";
        const chatbotScoreAvg = item.chatbotScoreAvg ? formatChatbotScore(item.chatbotScoreAvg, 1) : "";
        const chatbotDetailParts = [];
        if (chatbotStarted && chatbotProfile) {
          if (chatbotProfile.role) chatbotDetailParts.push(chatbotRoleLabel(chatbotProfile.role));
          if (Number.isFinite(item.chatbotTime)) chatbotDetailParts.push(`เวลาเฉลี่ย ${formatDurationShort(item.chatbotTime)}`);
          if (Number(item.chatbotAttempts) > 0) chatbotDetailParts.push(`ทำ ${formatChatbotNumber(item.chatbotAttempts, 0)} ครั้ง`);
          if (Number.isFinite(Number(chatbotProfile.percentile))) {
            chatbotDetailParts.push(`เปอร์เซ็นไทล์ ${formatChatbotNumber(chatbotProfile.percentile)}`);
          }
          if (Number.isFinite(Number(chatbotProfile.cohortLearners))) {
            chatbotDetailParts.push(`เทียบผู้เรียน ${formatChatbotNumber(chatbotProfile.cohortLearners, 0)} คน`);
          }
        } else if (chatbotStarted) {
          if (Number.isFinite(item.chatbotTime)) chatbotDetailParts.push(`ใช้เวลา ${formatDurationShort(item.chatbotTime)}`);
          if (Number.isFinite(item.chatbotTimeAvg)) chatbotDetailParts.push(`เฉลี่ย ${formatDurationShort(item.chatbotTimeAvg)}`);
        }
        const chatbotTimeText = chatbotDetailParts.join(" • ");
        const itemDetail = isChatbot
          ? (chatbotStarted ? chatbotTimeText : "ยังไม่ได้เริ่ม")
          : isVk
            ? (vk?.hasActivity
              ? `ใช้เวลา ${formatVkMinutes(vk?.metrics?.["Time used"])}`
              : "ยังไม่มีข้อมูลการใช้งาน VK")
          : (reading
        ...
        const chatbotScoreHero = isChatbot && chatbotStarted
          ? `<div class="shrink-0 text-right">
              ...
              ${chatbotScoreAvg ? `<div class="mt-1 text-xs text-slate-500">${chatbotProfile ? "เฉลี่ยกลุ่ม" : "เฉลี่ย"} ${escapeHtml(chatbotScoreAvg)}</div>` : ""}
            </div>`
          : "";
        const vkScoreParts = isVk && vk?.hasActivity
          ? [
              vk?.metrics?.["Coding mark"] != null ? `Coding ${formatVkMark(vk.metrics["Coding mark"])}` : "",
              vk?.metrics?.["Solution mark"] != null ? `Solution ${formatVkMark(vk.metrics["Solution mark"])}` : ""
            ].filter(Boolean)
          : [];
        const vkScoreHero = vkScoreParts.length
          ? `<div class="shrink-0 text-right">
              <div class="text-[11px] uppercase tracking-wide text-slate-400">ผลการใช้ VK</div>
              <div class="text-sm font-extrabold text-violet-700 mono leading-tight">${escapeHtml(vkScoreParts.join(" • "))}</div>
            </div>`
          : "";
        const vkMetricParts = isVk && vk?.hasActivity
          ? [
              Number(vk?.metrics?.["VK time"]) > 0 ? `เวลาใน VK ${formatVkMinutes(vk.metrics["VK time"])}` : "",
              Number(vk?.metrics?.["Switch count"]) > 0 ? `สลับ IDE/VK ${Number(vk.metrics["Switch count"])} ครั้ง` : "",
              Number(vk?.metrics?.["Scene used"]) > 0 ? `Scene ${Number(vk.metrics["Scene used"])}` : "",
              Number(vk?.metrics?.["Object used"]) > 0 ? `Object ${Number(vk.metrics["Object used"])}` : ""
            ].filter(Boolean)
          : [];
        const vkMetricsHtml = vkMetricParts.length
          ? `<div class="mt-2 flex flex-wrap gap-1.5">${vkMetricParts.map((part) => `<span class="rounded-full bg-violet-50 border border-violet-100 px-2 py-0.5 text-[11px] text-violet-700">${escapeHtml(part)}</span>`).join("")}</div>`
          : "";
        ...
        return `
          <div class="rounded-xl bg-white/70 px-3 py-2 border border-slate-200">
            <div class="flex items-start justify-between gap-3">
              ...
              ${chatbotScoreHero}
              ${vkScoreHero}
              ${(!isChatbot && !isVk && itemHasPct) ? `<div class="text-xs font-bold mono shrink-0 ${pctToneClass}">${escapeHtml(itemPctText)}</div>` : ""}
            </div>
            ${itemHeatmapHtml}
            ${itemProgressHtml}
            ${vkMetricsHtml}
...
const fetchCourseDetail = async () => {
  ...
  try {
    ...
    if (!window.courseToolInventory?.hasVk) {
      window.vkOverviewRawData = null;
      window.vkChapterRawData = [];
      window.vkItemData = [];
    }
    syncOverallToolCards();
    const acc = { chapter: 0, sequential: 0, vertical: 0, html: 0 };
...
const fetchVkLearningData = async () => {
  window.vkOverviewRawData = null;
  window.vkChapterRawData = [];
  window.vkItemData = [];
  ...
  if (!userId || !courseId || !isLikelyCourseId(courseId)) {
    requestDashboardRender();
    setDebugApiEntry("vk-analysis", {
      label: "ผลการใช้งาน VK Tool",
      state: "skipped",
      badge: "ไม่มีข้อมูล",
      message: "ยังไม่พร้อมแสดงข้อมูล VK",
      requests: []
    });
    return { state: "skipped", message: "ยังไม่พร้อมแสดงข้อมูล VK" };
  }

  const overviewSrcUrl = vkAnalysisOverviewUrl(userId, courseId);
  const debugRequests = [];
  try {
    const overviewRes = await fetch(overviewSrcUrl);
    if (!overviewRes.ok) throw await createHttpError(overviewRes);
    const overview = await overviewRes.json();
    window.vkOverviewRawData = overview;
    debugRequests.push({
      label: "ภาพรวมรายวิชา",
      url: overviewSrcUrl,
      state: "success",
    ...
    const chapterNumbers = Array.from(new Set(
      (Array.isArray(overview?.Params?.x_data) ? overview.Params.x_data : [])
        .map((value) => Number(value))
        .filter((value) => Number.isFinite(value) && value > 0)
    ));
    const chapterResults = await Promise.allSettled(chapterNumbers.map(async (chapterNo) => {
      const srcUrl = vkAnalysisOverviewUrl(userId, courseId, chapterNo);
      const res = await fetch(srcUrl);
      if (!res.ok) throw await createHttpError(res);
      const payload = await res.json();
      return { chapterNo, srcUrl, payload };
    }));
    ...
    chapterResults.forEach((result, index) => {
      ...
      if (result.status === "fulfilled") {
        chapterPayloads.push(result.value.payload);
        items.push(...normalizeVkChapterItems(result.value.payload, chapterNo));
        debugRequests.push({
          label: `บทวิเคราะห์ VK ${chapterNo}`,
          url: result.value.srcUrl,
          state: "success",
          message: "พร้อมแสดงผล",
        ...
        debugRequests.push({
          ...
          state: "error",
          message: "มีปัญหา",
          payload: result.reason?.message || "เกิดข้อผิดพลาด"
        });
      }
    });

    window.vkChapterRawData = chapterPayloads;
    window.vkItemData = items;
    const failedChapters = chapterResults.filter((result) => result.status === "rejected").length;
    const message = failedChapters
      ? `พบข้อมูล VK ${items.length} กิจกรรม • โหลดบทไม่สำเร็จ ${failedChapters} บท`
      : `พบข้อมูล VK ${items.length} กิจกรรม`;
    setDebugApiEntry("vk-analysis", {
      label: "ผลการใช้งาน VK Tool",
      state: "success",
      badge: "พร้อมแสดงผล",
    ...
    window.vkChapterRawData = [];
    window.vkItemData = [];
    console.warn("VK Analysis API error:", err);
    setDebugApiEntry("vk-analysis", {
      label: "ผลการใช้งาน VK Tool",
      state: "error",
      badge: "มีปัญหา",
      message: "ไม่สามารถแสดงข้อมูล VK ได้ในขณะนี้",
      requests: [{
        label: "ภาพรวมรายวิชา",
        url: overviewSrcUrl,
        state: "error",
    ...
    return { state: "error", message: "ไม่สามารถแสดงข้อมูล VK ได้ในขณะนี้" };
  } finally {
    requestDashboardRender();
  }
};
...
const fetchChatbotData = async () => {
  ...
  const canRequestProfile = Boolean(
    ...
    accessToken &&
    courseId &&
    isLikelyCourseId(courseId)
  );

  window.chatbotProfileRawData = null;
  window.chatbotDataSource = null;
  window.chatbotPerfData = null;
  window.chatbotSpeedRawData = null;

  if (canRequestProfile) {
    const srcUrl = chatbotProfileUrl(learnerEmail, courseId);
    try {
      const res = await fetch(srcUrl, {
        headers: { Authorization: `Bearer ${accessToken}` }
      });
      if (res.status === 401) {
        showSessionExpired();
        const message = "เซสชันหมดอายุ กรุณาเข้าสู่ระบบอีกครั้ง";
        setDebugApiEntry("chatbot-profile", {
          label: "ผลการทำแบบฝึกหัด",
          state: "error",
          badge: "มีปัญหา",
          message,
          requests: [
            { label: "Chatbot API ใหม่", url: srcUrl, state: "error", message, payload: "HTTP 401" }
          ]
        });
        rebuildChatbotOrderMap();
        requestDashboardRender();
      ...
      if (!res.ok) throw await createHttpError(res);
      const data = await res.json();
      const normalized = normalizeChatbotProfileData(data);

      if (normalized.rows.length) {
        window.chatbotProfileRawData = data;
        window.chatbotDataSource = "profile";
        window.chatbotPerfData = normalized.performance;
        window.chatbotSpeedRawData = normalized.speed;
        setDebugApiEntry("chatbot-profile", {
          label: "ผลการทำแบบฝึกหัด",
          state: "success",
          badge: "พร้อมแสดงผล",
          message: `ใช้ Chatbot API ใหม่ • พบ ${normalized.rows.length} Quiz`,
          requests: [
            { label: "Chatbot API ใหม่", url: srcUrl, state: "success", message: "พร้อมแสดงผล", payload: data }
          ]
        });
        setDebugApiEntry("chatbot-speed", {
          label: "ระยะเวลาการทำแบบฝึกหัด",
          state: "skipped",
          badge: "ใช้ API ใหม่",
          message: "ไม่เรียก API เดิม เพราะพบข้อมูลจาก Chatbot API ใหม่",
          requests: []
        });
        setDebugApiEntry("chatbot-performance", {
          label: "ผลการทำแบบฝึกหัด (API เดิม)",
          state: "skipped",
          badge: "ใช้ API ใหม่",
          message: "ไม่เรียก API เดิม เพราะพบข้อมูลจาก Chatbot API ใหม่",
          requests: []
        ...
        return {
          state: "success",
          message: `ใช้ข้อมูล Chatbot API ใหม่ • พบ ${normalized.rows.length} Quiz`
        };
      }
      ...
      setDebugApiEntry("chatbot-profile", {
        ...
        state: "skipped",
        badge: "ใช้ API เดิม",
        message: "ไม่พบ Quiz ที่เคยทำใน API ใหม่ จึงเรียก API เดิม",
        requests: [
          { label: "Chatbot API ใหม่", url: srcUrl, state: "skipped", message: "ไม่พบข้อมูลผู้เรียน", payload: data }
        ]
      });
    } catch (err) {
      console.warn("Chatbot profile API error; falling back to legacy API:", err);
      setDebugApiEntry("chatbot-profile", {
        label: "ผลการทำแบบฝึกหัด",
        state: "error",
        badge: "ใช้ API เดิม",
        message: "API ใหม่ไม่พร้อมใช้งาน จึงเรียก API เดิม",
        requests: [
          { label: "Chatbot API ใหม่", url: srcUrl, state: "error", message: "มีปัญหา", payload: err?.message || "เกิดข้อผิดพลาด" }
        ]
      });
    }
  } else {
    const reason = !courseId || !isLikelyCourseId(courseId)
      ? "ไม่มี courseid"
      : (!learnerEmail ? "ไม่พบ learner email" : "ไม่พบ access token");
    setDebugApiEntry("chatbot-profile", {
      label: "ผลการทำแบบฝึกหัด",
      state: "skipped",
      badge: "ใช้ API เดิม",
      message: `ยังเรียก API ใหม่ไม่ได้เพราะ${reason} จึงเรียก API เดิม`,
      requests: []
    });
  }

  window.chatbotDataSource = "legacy";
  const [speedResult, performanceResult] = await Promise.all([
    fetchChatbotSpeed(),
    fetchChatbotPerformance()
  ]);
  const legacySuccess = performanceResult?.state === "success" || speedResult?.state === "success";
  if (legacySuccess) {
    return { state: "success", message: "ไม่พบข้อมูลจาก API ใหม่ • ใช้ข้อมูล Chatbot API เดิม" };
  }
  if (performanceResult?.state === "error" || speedResult?.state === "error") {
    return { state: "error", message: "ไม่พบข้อมูลจาก API ใหม่ และไม่สามารถโหลด API เดิมได้" };
  }
  return { state: "skipped", message: "ไม่พบข้อมูล Chatbot จากทั้ง API ใหม่และ API เดิม" };
};
...
const getDashboardTaskDefs = () => {
  const tasks = [
    ...
    { id: "vk-analysis", label: "ผลการใช้งาน VK Tool", run: fetchVkLearningData, requiresTool: "vk" },
    { id: "chatbot-data", label: "ผลการทำแบบฝึกหัด", run: fetchChatbotData, requiresTool: "chatbot" },
