...
const chatbotQuizUrl = (email, usageId) => `${teacherConfig.baseUrl}/api/kidbright/course/${encodeURIComponent(usageId)}/data/bookroll?email=${encodeURIComponent(email)}`;
...
async function loadStudentDetailApis(st) {
  ...
  const loadReading = async () => {
    ...
    try {
      const payload = await apiGet(bookrollReadingUrl(email, cid));
    ...
    if (!entries.length && !courseLevelAnswered) {
      ...
      const responses = await Promise.allSettled(targets.map((target) => apiGet(bookrollReadingUrl(email, target.usageId))));
