...
| ~~`GET`~~ | ~~`{BASEURL}/api/kidbright/course/{usageId}/data/bookroll?email={email}`~~ | **ตอบ 404 — ยังไม่ได้ deploy** (ทดสอบ 2026-07-23 ทั้ง prod/dev) ใช้ BookRoll ตรงในตาราง Design Reference แทน | - |
...
| `GET {BASEURL}/api/kidbright/course/{usageId}/data/bookroll?email={email}` | ตั้งใจให้เป็น proxy ดึงความคืบหน้าการอ่าน BookRoll ผ่าน MECA **แต่ยังไม่มีอยู่จริง — ตอบ 404** ทดสอบ 2026-07-23 ทั้ง `adaptive-profile-bn` และ `adaptive-profile-bn-dev` ทั้งแบบส่ง courseId และ aetool block id (บน dev ตอบ 404 ตั้งแต่ยังไม่แนบ token ขณะที่ `/course/{courseId}` ตอบ 401 แปลว่าไม่มี route) `teacher.js` จึงยิง `bookroll.thaidlt.com` ตรงแทน ถ้า backend deploy เส้นนี้เมื่อไหร่ค่อยสลับกลับได้ | 404 ยืนยันแล้ว |
...
| Bookroll reading data | `https://bookroll.thaidlt.com/meca/student/readingData?userID={uid}&usageId={courseId}&view=student&ts={timestamp}` | `https://bookroll.thaidlt.com/meca/student/readingData?userID={uid}&usageId=course-v1%3ANECTEC%2BAIUPPERSECONDARY01%2BNECTEC_000006&view=student` | ดึงความคืบหน้าการอ่าน BookRoll ใช้ในลิ้นชักผู้เรียนของ teacher dashboard **✅ ใช้งานอยู่จริง** ยืนยัน 2026-07-23: `userID` รับ **email ของผู้เรียนได้เลย** ไม่ต้องใช้ Keycloak userId, ตอบ `access-control-allow-origin: *` จึงไม่ติด CORS, ไม่ต้องมี token `usageId` ใส่ได้ทั้ง courseId และ aetool block id ตอบกลับเป็น `{"results":{"<ชื่อเอกสาร>":"<อ่านแล้ว>:<ทั้งหมด>"}}` เช่น `"2.3 train":"10:10"` โดย `"0:0"` = ยังไม่เปิดอ่าน (นับเป็น 0%) |
